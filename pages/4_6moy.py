...
df = pd.read_csv('https://raw.githubusercontent.com/Schemme/FINAL-PROJECT/main/sixth-man.csv')
...
check_1 = st.sidebar.checkbox("1st Votes", value=False)
check_2 = st.sidebar.checkbox("2nd Votes", value=False)
check_3 = st.sidebar.checkbox("3rd Votes", value=False)
...
if columns_to_display:
    ...
    fig = px.bar(grouped_data_without_player, barmode='group', color_discrete_map=colors)
    fig.update_layout(plot_bgcolor='#27292B', title='Votes Distribution', xaxis_title='Player', yaxis_title='Votes')
    st.plotly_chart(fig)
else:
