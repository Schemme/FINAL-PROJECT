...
def plot_stats(data):
    ...
    # Plotting WIN%
    axs[1].bar(x_positions, data['WIN%'], color='grey', alpha=0.7)
...
if len(division_df) > 0:
    ...
    st.write("""This model allows us to see the correlation between oEFF and PPG for a team and their resulting win percentage. Obviously offense is needed to win games but it's important to see if the teams that are scoring more, and doing so efficiently, are winning more. This also allows us to see which teams are performing the best in their division.""") 
